...
export class EstructurasService {
  private apiUrl = 'http://localhost:3000/api/estructuras';
  ...
  crearEstructura(estructura: any): Observable<any> {
    const url = 'http://localhost:3000/api/estructuras/nuevo';
    return this.http.post<any>(url, estructura);
  }

  getOrigenes(): Observable<any[]> {
    const url = 'http://localhost:3000/api/estructuras/origen';
    return this.http.get<any[]>(url);
  }

  getMarcas(): Observable<any[]> {
    const url = 'http://localhost:3000/api/estructuras/marca';
    return this.http.get<any[]>(url);
  }

  softDeleteEstructura(id: number): Observable<any> {
    const url = `http://localhost:3000/api/estructuras/${id}`;
    return this.http.delete<any>(url);
  }
  
  reactivarEstructura(id: number): Observable<any> {
    const url = `http://localhost:3000/api/estructuras/${id}/reactivar`;
